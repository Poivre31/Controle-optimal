...
double target_altitude = 20000;
size_t max_steps = 10000;
double target_precision = 1e-10;

double Mi = 1e3;            ///> Initial mass of the rocket
double Mf = Mi / 10;        ///> Final mass, at end of burn
double thrust = 2 * g * Mi; ///> Thrust (here initial TWR of 2)
double A = thrust / 3000;   ///> Fuel consumption rate (here with an ISP of 3000m/s)
...
int main()
{
    ...
    auto loss = [&solver](parameters c)
    {
        ...
        double loss = (result[Z] / target_altitude - 1) * (result[Z] / target_altitude - 1) + result[Vz] * result[Vz] / (8000 * 8000);
        return loss;
    };
    ...
    if (true)
    {
        ...
        plt::title("Horizontal velocity Vx(t)");
